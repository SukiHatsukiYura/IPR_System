...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'save_agency') {
        try {
            ...
            // 先删除该专利案件的所有代理机构记录（因为现在是单选模式）
            $delete_stmt = $pdo->prepare("DELETE FROM patent_case_agency WHERE patent_case_info_id = ?");
            $delete_stmt->execute([$patent_id]);
                ...
                foreach ($agent_id_array as $agent_id) {
                    if (!empty($agent_id)) {
                        ...
                        $insert_stmt = $pdo->prepare("
                            INSERT INTO patent_case_agency (
                                patent_case_info_id, agency_id, agency_agent_id, 
            ...
            if (!empty($contact_id_array)) {
                foreach ($contact_id_array as $contact_id) {
                    if (!empty($contact_id)) {
                        ...
                        $insert_stmt = $pdo->prepare("
                            INSERT INTO patent_case_agency (
                                patent_case_info_id, agency_id, agency_contact_id,
            ...
            if (empty($agent_id_array) && empty($contact_id_array)) {
                $insert_stmt = $pdo->prepare("
                    INSERT INTO patent_case_agency (
                        patent_case_info_id, agency_id, agency_name_cn, agency_code, remark
                    ) VALUES (?, ?, ?, ?, ?)
                ");
                $insert_stmt->execute([
                    $patent_id,
    ...
    if ($action === 'delete_agency') {
        try {
            ...
            $stmt = $pdo->prepare("DELETE FROM patent_case_agency WHERE id = ? AND patent_case_info_id = ?");
            $result = $stmt->execute([$id, $patent_id]);
    ...
    if ($action === 'load_agency') {
        try {
            // 加载专利案件的代理机构信息
            $sql = "SELECT pca.*, a.agency_name_cn, a.agency_code 
                    FROM patent_case_agency pca 
                    LEFT JOIN agency a ON pca.agency_id = a.id 
                    WHERE pca.patent_case_info_id = ?";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$patent_id]);
            $agencies = $stmt->fetchAll();
            ...
            if (!empty($agencies)) {
                ...
                $agent_sql = "SELECT aa.id, aa.name_cn, aa.license_no, aa.phone 
                             ...
                             WHERE aa.agency_id = ? AND aa.id IN (
                                 SELECT agency_agent_id FROM patent_case_agency 
                                 WHERE patent_case_info_id = ? AND agency_agent_id IS NOT NULL
                             )";
                $agent_stmt = $pdo->prepare($agent_sql);
                $agent_stmt->execute([$agency['agency_id'], $patent_id]);
                ...
                $contact_sql = "SELECT ac.id, ac.name, ac.mobile, ac.work_email 
                               ...
                               WHERE ac.agency_id = ? AND ac.id IN (
                                   SELECT agency_contact_id FROM patent_case_agency 
                                   WHERE patent_case_info_id = ? AND agency_contact_id IS NOT NULL
                               )";
                $contact_stmt = $pdo->prepare($contact_sql);
                $contact_stmt->execute([$agency['agency_id'], $patent_id]);
