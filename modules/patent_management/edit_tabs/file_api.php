...
if (!isset($_GET['patent_id']) || intval($_GET['patent_id']) <= 0) {
    header('Content-Type: application/json');
    echo json_encode(['success' => false, 'message' => '未指定专利ID']);
    exit;
}
...
if (!$patent_stmt->fetch()) {
    header('Content-Type: application/json');
    echo json_encode(['success' => false, 'message' => '未找到该专利信息']);
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'upload') {
        try {
            ...
            // 验证文件类型
            $allowed_types = ['申请书', '说明书', '权利要求书', '附图', '审查意见', '答复意见', '其他'];
            if (!in_array($file_type, $allowed_types)) {
                throw new Exception('无效的文件类型');
            }

            // 创建上传目录
            $upload_dir = __DIR__ . '/../../../uploads/patent_files/' . $patent_id . '/';
            ...
            // 保存到数据库
            $relative_path = 'uploads/patent_files/' . $patent_id . '/' . $final_filename;

            $stmt = $pdo->prepare("
                INSERT INTO patent_case_file (
                    patent_case_info_id, file_type, file_name, file_path, 
    ...
    if ($action === 'list') {
        try {
            ...
            $sql = "SELECT pcf.*, u.real_name as uploader_name 
                    FROM patent_case_file pcf 
                    LEFT JOIN user u ON pcf.upload_user_id = u.id 
                    WHERE pcf.patent_case_info_id = ?";
            $params = [$patent_id];

            if (!empty($file_type)) {
                $sql .= " AND pcf.file_type = ?";
                $params[] = $file_type;
            }

            $sql .= " ORDER BY pcf.created_at DESC";
    ...
    if ($action === 'delete') {
        try {
            ...
            // 获取文件信息
            $stmt = $pdo->prepare("SELECT file_path FROM patent_case_file WHERE id = ? AND patent_case_info_id = ?");
            $stmt->execute([$file_id, $patent_id]);
            ...
            // 删除数据库记录
            $stmt = $pdo->prepare("DELETE FROM patent_case_file WHERE id = ? AND patent_case_info_id = ?");
            $result = $stmt->execute([$file_id, $patent_id]);
