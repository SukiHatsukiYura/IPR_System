...
$patent_case_applicant_id = intval($_REQUEST['patent_case_applicant_id'] ?? 0);
$file_type = $_REQUEST['file_type'] ?? '';

// 上传目录
$upload_dir = __DIR__ . '/../../../uploads/patent_applicant_files/';
...
if ($action === 'upload') {
    // 检查必要参数
    if (!$patent_case_applicant_id || !$file_type) {
    ...
    // 验证申请人是否存在
    $stmt = $pdo->prepare("SELECT patent_case_info_id FROM patent_case_applicant WHERE id = ?");
    $stmt->execute([$patent_case_applicant_id]);
    ...
    if (move_uploaded_file($file['tmp_name'], $file_path)) {
        ...
        $stmt = $pdo->prepare("INSERT INTO patent_case_applicant_file 
            (patent_case_applicant_id, patent_case_info_id, file_type, file_name, file_path, file_size, mime_type, upload_user_id) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)");

        $result = $stmt->execute([
            $patent_case_applicant_id,
            $applicant['patent_case_info_id'],
            $file_type,
            $display_filename, // 使用显示文件名（可能是自定义的）
            'uploads/patent_applicant_files/' . $unique_filename,
            $file_size,
            $mime_type,
            $_SESSION['user_id']
        ]);
    ...
    // 获取文件信息
    $stmt = $pdo->prepare("SELECT file_path FROM patent_case_applicant_file WHERE id = ?");
    $stmt->execute([$file_id]);
    $file = $stmt->fetch();

    if ($file) {
        // 删除数据库记录
        $stmt = $pdo->prepare("DELETE FROM patent_case_applicant_file WHERE id = ?");
    ...
    $stmt = $pdo->prepare("SELECT id, file_name, file_path, file_size, created_at 
        FROM patent_case_applicant_file 
        WHERE patent_case_applicant_id = ? AND file_type = ? 
        ORDER BY created_at DESC");
    $stmt->execute([$patent_case_applicant_id, $file_type]);
    $files = $stmt->fetchAll();
