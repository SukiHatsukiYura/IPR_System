...
// 处理更新核稿状态功能AJAX请求
if (isset($_POST['action']) && in_array($_POST['action'], ['set_draft', 'set_under_review'])) {
    ...
    try {
        ...
        foreach ($selected_case_ids as $case_id) {
            ...
            // 更新核稿状态表中的状态（只更新已完成状态的记录）
            $update_stmt = $pdo->prepare("UPDATE patent_case_review_status SET review_status = ?, updated_at = NOW() WHERE patent_case_info_id = ? AND review_status = '已完成'");
...
<div class="module-panel">
    ...
    ?>
    <?php render_info_notice("已完成管理（当前用户：" . $current_user_name . "）：只显示核稿状态为已完成的专利案件", 'success', 'icon-search'); ?>
...
<script>
    (function() {
        ...
        window.loadPatentData = function() {
            ...
            var requestUrl = baseUrl.replace('index.php', '') + 'modules/patent_management/review_management/completed.php';
