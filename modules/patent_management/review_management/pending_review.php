...
if (isset($_POST['action']) && in_array($_POST['action'], ['set_under_review', 'set_completed'])) {
    ...
    try {
        ...
        foreach ($selected_case_ids as $case_id) {
            ...
            // 更新核稿状态表中的状态（只更新分配给当前用户的草稿状态记录）
            $update_stmt = $pdo->prepare("UPDATE patent_case_review_status SET review_status = ?, updated_at = NOW() WHERE patent_case_info_id = ? AND review_status = '草稿' AND reviewer_id = ?");
            $result = $update_stmt->execute([$new_status, $case_id, $user_id]);
...
if (isset($_GET['ajax']) && $_GET['ajax'] == 1) {
    ...
    // 获取核稿状态为"草稿"且核稿人为当前用户的案件ID列表
    $current_user_id = $_SESSION['user_id'];
    $draft_stmt = $pdo->prepare("SELECT DISTINCT patent_case_info_id FROM patent_case_review_status WHERE review_status = '草稿' AND reviewer_id = ?");
    $draft_stmt->execute([$current_user_id]);
    $draft_cases = $draft_stmt->fetchAll(PDO::FETCH_COLUMN);

    if (empty($draft_cases)) {
        // 没有分配给当前用户核稿的草稿状态案件
        echo json_encode([
            'success' => true,
            'html' => '<tr><td colspan="11" style="text-align:center;padding:20px 0;">暂无分配给您核稿的草稿状态案件</td></tr>',
            'total_records' => 0,
            'total_pages' => 1,
            'current_page' => 1
        ]);
    ...
    // 添加草稿案件的筛选条件
    $placeholders = str_repeat('?,', count($draft_cases) - 1) . '?';
    $where[] = "id IN ($placeholders)";
    $params = array_merge($params, $draft_cases);
...
<div class="module-panel">
    ...
    ?>
    <?php render_info_notice("待我核稿（当前用户：" . $current_user_name . "）：只显示分配给您核稿的草稿状态专利案件", 'success', 'icon-search'); ?>
...
<script>
    (function() {
        ...
        window.loadPatentData = function() {
            ...
            var requestUrl = baseUrl.replace('index.php', '') + 'modules/patent_management/review_management/pending_review.php';
