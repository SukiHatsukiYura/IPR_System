...
// 草稿功能 - 专利管理/核稿管理模块下的草稿功能 - 显示核稿状态为草稿的专利案件
...
if (isset($_POST['action']) && in_array($_POST['action'], ['set_under_review', 'set_completed'])) {
    ...
    try {
        ...
        foreach ($selected_case_ids as $case_id) {
            ...
            // 更新核稿状态表中的状态
            $update_stmt = $pdo->prepare("UPDATE patent_case_review_status SET review_status = ?, reviewer_id = ?, updated_at = NOW() WHERE patent_case_info_id = ? AND review_status = '草稿'");
            $result = $update_stmt->execute([$new_status, $user_id, $case_id]);
...
if (isset($_GET['ajax']) && $_GET['ajax'] == 1) {
    ...
    // 获取核稿状态为"草稿"的案件ID列表
    $draft_stmt = $pdo->prepare("SELECT DISTINCT patent_case_info_id FROM patent_case_review_status WHERE review_status = '草稿'");
    $draft_stmt->execute();
    ...
    if (empty($draft_cases)) {
        ...
        echo json_encode([
            'success' => true,
            'html' => '<tr><td colspan="11" style="text-align:center;padding:20px 0;">暂无草稿状态的案件</td></tr>',
            'total_records' => 0,
            'total_pages' => 1,
            'current_page' => 1
        ]);
...
<div class="module-panel">
    ...
    ?>
    <?php render_info_notice("草稿管理（当前用户：" . $current_user_name . "）：只显示核稿状态为草稿的专利案件", 'success', 'icon-search'); ?>
...
<script>
    (function() {
        ...
        window.loadPatentData = function() {
            ...
            var requestUrl = baseUrl.replace('index.php', '') + 'modules/patent_management/review_management/draft.php';
