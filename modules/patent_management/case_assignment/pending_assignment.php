...
// 处理置为已配案功能AJAX请求
if (isset($_POST['action']) && $_POST['action'] == 'set_allocated') {
    ...
    if (empty($case_ids)) {
        echo json_encode(['success' => false, 'msg' => '请选择要设置为已配案的案件']);
        exit;
    }
    ...
    try {
        ...
        foreach ($case_ids_array as $case_id) {
            ...
            // 检查案件是否存在且未配案
            $check_stmt = $pdo->prepare("SELECT id, case_code FROM patent_case_info WHERE id = ? AND is_allocated = 0");
            $check_stmt->execute([$case_id]);
            $case_info = $check_stmt->fetch();

            if ($case_info) {
                // 更新为已配案
                $update_stmt = $pdo->prepare("UPDATE patent_case_info SET is_allocated = 1, updated_at = NOW() WHERE id = ?");
        ...
        $pdo->commit();
        echo json_encode(['success' => true, 'msg' => "成功将 {$success_count} 个案件设置为已配案"]);
    } catch (Exception $e) {
        $pdo->rollBack();
        echo json_encode(['success' => false, 'msg' => '设置已配案失败: ' . $e->getMessage()]);
    }
    exit;
}
...
<div class="module-panel">
    <div class="module-btns" style="display: flex; flex-direction: column; gap: 10px;">
        <div style="display: flex; gap: 8px; flex-wrap: wrap;">
            ...
            <button type="button" class="btn-set-allocated btn-mini" disabled><i class="icon-save"></i> 置为已配案</button>
        </div>
    </div>
    <?php render_info_notice("待配案：只显示未配案的专利案件", 'warning', 'icon-list'); ?>
...
<script>
    (function() {
        var form = document.getElementById('search-form'),
            ...
            btnSetAllocated = document.querySelector('.btn-set-allocated'),
        ...
        window.loadPatentData = function() {
            ...
            var requestUrl = baseUrl.replace('index.php', '') + 'modules/patent_management/case_assignment/pending_assignment.php';
        ...
        function updateAllocatedButtonState() {
            var checkedCount = patentList.querySelectorAll('.case-checkbox:checked').length;
            btnSetAllocated.disabled = checkedCount === 0;
        }

        btnSetAllocated.onclick = function() {
            var checkedBoxes = patentList.querySelectorAll('.case-checkbox:checked');
            if (checkedBoxes.length === 0) {
                alert('请先选择要设置为已配案的案件');
                return;
            }
            ...
            if (confirm('确定要将选中的 ' + checkedBoxes.length + ' 个案件设置为已配案吗？')) {
                var xhr = new XMLHttpRequest();
                var baseUrl = window.location.href.split('?')[0];
                var requestUrl = baseUrl.replace('index.php', '') + 'modules/patent_management/case_assignment/pending_assignment.php';
                        ...
                        if (xhr.status === 200) {
                            try {
                                ...
                                if (response.success) {
                                    ...
                                    alert('设置已配案失败：' + response.msg);
                                }
                            } catch (e) {
                                alert('设置已配案失败：服务器响应错误');
                            }
                        } else {
                            alert('设置已配案失败：网络错误');
                        }
                ...
                xhr.send('action=set_allocated&case_ids=' + encodeURIComponent(caseIds));
            }
        };
