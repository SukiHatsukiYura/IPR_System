...
if (isset($_GET['ajax']) && $_GET['ajax'] == 1) {
    ...
    if (!empty($_GET['handler_id'])) {
        $where[] = 'handler_id = ?';
        $params[] = $_GET['handler_id'];
    }
...
<div class="module-panel">
    ...
    <form id="search-form" class="module-form" autocomplete="off">
        ...
        ?>
        <?php render_info_notice("部门案件查询（当前用户：" . $current_user_name . "）：只显示您作为对应部门的项目负责人的专利案件", 'success', 'icon-search'); ?>
...
<script>
    (function() {
        ...
        window.loadPatentData = function() {
            ...
            var requestUrl = baseUrl.replace('index.php', '') + 'modules/patent_management/personal_cases/department_cases.php';
                    ...
                    if (xhr.status === 200) {
                        try {
                            ...
                            if (response.success) {
                                ...
                                bindTableRowClick();
                                console.log('部门案件查询 - 数据加载完成，总记录数:', response.total_records);
                            } else {
                                console.error('部门案件查询 - 请求失败:', response);
                                patentList.innerHTML = '<tr><td colspan="10" style="text-align:center;padding:20px 0;">加载数据失败</td></tr>';
                            }
                        } catch (e) {
                            console.error('部门案件查询 - 解析响应失败:', e, xhr.responseText);
                            patentList.innerHTML = '<tr><td colspan="10" style="text-align:center;padding:20px 0;">加载数据失败</td></tr>';
                        }
                    } else {
        ...
        btnEdit.onclick = function() {
            ...
            sessionStorage.setItem('patent_edit_source_submenu', '4');
