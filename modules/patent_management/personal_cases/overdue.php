...
// 已逾期案件功能 - 专利管理/个人案件模块下的已逾期案件管理功能
...
if (isset($_GET['ajax']) && $_GET['ajax'] == 1) {
    ...
    if ($follow_info_data) {
        // 直接查询状态为"已逾期"的案件ID
        $status_stmt = $pdo->prepare("SELECT patent_case_id FROM user_patent_follow_case_status WHERE user_patent_follow_id = ? AND case_status = '已逾期'");
    ...
    if (empty($overdue_case_ids)) {
        echo json_encode([
            'success' => true,
            'html' => '<tr><td colspan="10" style="text-align:center;padding:20px 0;">暂无已逾期的案件</td></tr>',
            'total_records' => 0,
            'total_pages' => 1,
            'current_page' => 1
        ]);
...
<div class="module-panel">
    ...
    ?>
    <?php render_info_notice("已逾期案件（当前用户：" . $current_user_name . "）：只显示您关注的已逾期专利案件", 'error', 'icon-search'); ?>
...
<script>
    (function() {
        ...
        window.loadPatentData = function() {
            ...
            var requestUrl = baseUrl.replace('index.php', '') + 'modules/patent_management/personal_cases/overdue.php';
