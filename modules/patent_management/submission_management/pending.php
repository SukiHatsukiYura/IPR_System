...
// 自动同步核稿状态和递交状态
function syncReviewAndSubmissionStatus($pdo)
{
    try {
        ...
        $completed_case_ids = $completed_stmt->fetchAll(PDO::FETCH_COLUMN);

        // 2. 获取所有核稿状态不是已完成的案件ID
        $not_completed_stmt = $pdo->prepare("SELECT DISTINCT patent_case_info_id FROM patent_case_review_status WHERE review_status != '已完成'");
        $not_completed_stmt->execute();
        $not_completed_case_ids = $not_completed_stmt->fetchAll(PDO::FETCH_COLUMN);

        // 3. 删除核稿状态不是已完成的案件的递交状态记录
        if (!empty($not_completed_case_ids)) {
            $placeholders = str_repeat('?,', count($not_completed_case_ids) - 1) . '?';
            $delete_stmt = $pdo->prepare("DELETE FROM patent_case_submission_status WHERE patent_case_info_id IN ($placeholders)");
            $delete_stmt->execute($not_completed_case_ids);
        }

        // 4. 为核稿状态为已完成但尚未存在递交状态记录的案件创建待处理记录
        if (!empty($completed_case_ids)) {
            foreach ($completed_case_ids as $case_id) {
                // 检查是否已经存在递交状态记录
                $check_stmt = $pdo->prepare("SELECT id FROM patent_case_submission_status WHERE patent_case_info_id = ?");
                $check_stmt->execute([$case_id]);

                if (!$check_stmt->fetch()) {
                    // 查找该案件的第一个处理事项ID
                    $task_stmt = $pdo->prepare("SELECT id FROM patent_case_task WHERE patent_case_info_id = ? ORDER BY id ASC LIMIT 1");
                    $task_stmt->execute([$case_id]);
                    $task_result = $task_stmt->fetch();

                    if ($task_result) {
                        // 插入新的递交状态记录
                        $insert_stmt = $pdo->prepare("INSERT INTO patent_case_submission_status (patent_case_info_id, patent_case_task_id, submission_status, reviewer_id, created_at, updated_at) VALUES (?, ?, '待处理', NULL, NOW(), NOW())");
                        $insert_stmt->execute([$case_id, $task_result['id']]);
                    }
                }
            }
        }
...
// 在页面加载时自动同步状态
syncReviewAndSubmissionStatus($pdo);

// 处理更新递交状态功能AJAX请求
if (isset($_POST['action']) && in_array($_POST['action'], ['set_under_review', 'set_completed'])) {
    ...
    try {
        ...
        foreach ($selected_case_ids as $case_id) {
            ...
            // 更新递交状态表中的状态（只更新待处理状态的记录）
            $update_stmt = $pdo->prepare("UPDATE patent_case_submission_status SET submission_status = ?, updated_at = NOW() WHERE patent_case_info_id = ? AND submission_status = '待处理'");
...
if (isset($_GET['ajax']) && $_GET['ajax'] == 1) {
    ...
    // 获取递交状态为"待处理"的案件ID列表
    $pending_stmt = $pdo->prepare("SELECT DISTINCT patent_case_info_id FROM patent_case_submission_status WHERE submission_status = '待处理'");
    $pending_stmt->execute();
    $pending_cases = $pending_stmt->fetchAll(PDO::FETCH_COLUMN);

    if (empty($pending_cases)) {
        // 没有待处理状态的案件
        echo json_encode([
            'success' => true,
            'html' => '<tr><td colspan="11" style="text-align:center;padding:20px 0;">暂无待处理状态的案件</td></tr>',
            'total_records' => 0,
            'total_pages' => 1,
            'current_page' => 1
        ]);
    ...
    // 添加待处理案件的筛选条件
    $placeholders = str_repeat('?,', count($pending_cases) - 1) . '?';
    $where[] = "id IN ($placeholders)";
    $params = array_merge($params, $pending_cases);
...
<div class="module-panel">
    <div class="module-btns" style="display: flex; flex-direction: column; gap: 10px;">
        ...
        <div style="display: flex; gap: 8px; flex-wrap: wrap; margin-top: 8px;">
            ...
            <button type="button" class="btn-set-under-review btn-mini" disabled style="background:#ff9800;color:#fff;border-color:#ff9800;">置为审核中</button>
            <button type="button" class="btn-set-completed btn-mini" disabled style="background:#4caf50;color:#fff;border-color:#4caf50;">置为已完成</button>
        </div>
    </div>
    <?php
    // 获取当前用户信息
    ...
    ?>
    <?php render_info_notice("待处理管理（当前用户：" . $current_user_name . "）：只显示递交状态为待处理的专利案件", 'success', 'icon-search'); ?>
...
<script>
    (function() {
        var form = document.getElementById('search-form'),
            ...
            btnSetUnderReview = document.querySelector('.btn-set-under-review'),
        ...
        window.loadPatentData = function() {
            ...
            btnSetUnderReview.disabled = true;
            ...
            var requestUrl = baseUrl.replace('index.php', '') + 'modules/patent_management/submission_management/pending.php';
        ...
        function updateFollowButtonState() {
            var checkedCount = patentList.querySelectorAll('.case-checkbox:checked').length;
            btnSetUnderReview.disabled = checkedCount === 0;
        ...
        btnEdit.onclick = function() {
            ...
            sessionStorage.setItem('patent_edit_source_menu', '4');
        ...
        function setReviewStatus(action, statusName) {
            ...
            if (confirm('确定要将选中的 ' + checkedBoxes.length + ' 个案件递交状态设置为 "' + statusName + '" 吗？')) {
                var xhr = new XMLHttpRequest();
                var baseUrl = window.location.href.split('?')[0];
                var requestUrl = baseUrl.replace('index.php', '') + 'modules/patent_management/submission_management/pending.php';
        ...
        // 置为审核中按钮事件
        btnSetUnderReview.onclick = function() {
            setReviewStatus('set_under_review', '审核中');
        };
