...
// 处理更新递交状态功能AJAX请求
if (isset($_POST['action']) && in_array($_POST['action'], ['set_pending', 'set_completed'])) {
    ...
    try {
        ...
        foreach ($selected_case_ids as $case_id) {
            ...
            // 更新递交状态表中的状态（只更新审核中状态的记录）
            $update_stmt = $pdo->prepare("UPDATE patent_case_submission_status SET submission_status = ?, updated_at = NOW() WHERE patent_case_info_id = ? AND submission_status = '审核中'");
...
if (isset($_GET['ajax']) && $_GET['ajax'] == 1) {
    ...
    // 获取递交状态为"审核中"的案件ID列表
    $under_review_stmt = $pdo->prepare("SELECT DISTINCT patent_case_info_id FROM patent_case_submission_status WHERE submission_status = '审核中'");
...
<div class="module-panel">
    ...
    ?>
    <?php render_info_notice("审核中管理（当前用户：" . $current_user_name . "）：只显示递交状态为审核中的专利案件", 'success', 'icon-search'); ?>
...
<script>
    (function() {
        ...
        window.loadPatentData = function() {
            ...
            var requestUrl = baseUrl.replace('index.php', '') + 'modules/patent_management/submission_management/under_review.php';
