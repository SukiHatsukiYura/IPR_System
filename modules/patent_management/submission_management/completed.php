...
// 处理更新递交状态功能AJAX请求
if (isset($_POST['action']) && in_array($_POST['action'], ['set_pending', 'set_under_review'])) {
    ...
    try {
        ...
        foreach ($selected_case_ids as $case_id) {
            ...
            // 更新递交状态表中的状态（只更新已完成状态的记录）
            $update_stmt = $pdo->prepare("UPDATE patent_case_submission_status SET submission_status = ?, updated_at = NOW() WHERE patent_case_info_id = ? AND submission_status = '已完成'");
...
if (isset($_GET['ajax']) && $_GET['ajax'] == 1) {
    ...
    // 获取递交状态为"已完成"的案件ID列表
    $completed_stmt = $pdo->prepare("SELECT DISTINCT patent_case_info_id FROM patent_case_submission_status WHERE submission_status = '已完成'");
    $completed_stmt->execute();
...
<div class="module-panel">
    <div class="module-btns" style="display: flex; flex-direction: column; gap: 10px;">
        ...
        <div style="display: flex; gap: 8px; flex-wrap: wrap; margin-top: 8px;">
            ...
            <button type="button" class="btn-set-pending btn-mini" disabled style="background:#ff9800;color:#fff;border-color:#ff9800;">置为待处理</button>
            <button type="button" class="btn-set-under-review btn-mini" disabled style="background:#2196f3;color:#fff;border-color:#2196f3;">置为审核中</button>
        </div>
    </div>
    <?php
    // 获取当前用户信息
    ...
    ?>
    <?php render_info_notice("已完成管理（当前用户：" . $current_user_name . "）：只显示递交状态为已完成的专利案件", 'success', 'icon-search'); ?>
...
<script>
    (function() {
        var form = document.getElementById('search-form'),
            ...
            btnSetPending = document.querySelector('.btn-set-pending'),
        ...
        window.loadPatentData = function() {
            ...
            var requestUrl = baseUrl.replace('index.php', '') + 'modules/patent_management/submission_management/completed.php';
        ...
        function updateFollowButtonState() {
            var checkedCount = patentList.querySelectorAll('.case-checkbox:checked').length;
            btnSetPending.disabled = checkedCount === 0;
        ...
        // 置为待处理按钮事件
        btnSetPending.onclick = function() {
            setReviewStatus('set_pending', '待处理');
        };
