...
try {
    ...
    // 批量导入数据
    $result = batchImportPatents($pdo, $rows, $header_map, $_SESSION['user_id']);
...
function getHeaderMapping($headers)
{
    ...
    $header_mapping = [
        ...
        '处理人' => 'handler_name', // 向后兼容
        '处理人ID' => 'handler_id', // 新格式
        '公告号' => 'announcement_no',
        ...
        '项目负责人' => 'project_leader_name', // 向后兼容
        '项目负责人ID' => 'project_leader_id', // 新格式
 ...
 * 批量导入专利案件
 */
function batchImportPatents($pdo, $rows, $header_map, $user_id)
{
    $success_count = 0;
    $error_count = 0;
    $errors = [];
    ...
    try {
        foreach ($rows as $row_index => $row) {
            ...
            try {
                ...
                // 验证必填字段
                $validation_result = validatePatentData($patent_data, $line_number);
                ...
                if (!empty($patent_data['case_code'])) {
                    $existing_id = checkExistingCase($pdo, $patent_data['case_code']);
                    if ($existing_id) {
                        $errors[] = "第{$line_number}行: 案件编号 {$patent_data['case_code']} 已存在";
                        $error_count++;
                        continue;
                    }
                }

                // 插入新案件（如果没有提供案件编号，会在insertPatentCase中自动生成）
                insertPatentCase($pdo, $patent_data, $user_id);
                $success_count++;
            } catch (Exception $e) {
 ...
 * 验证专利数据
 */
function validatePatentData($data, $line_number)
{
    // 根据add_patent.php的必填字段验证
    $required_fields = [
        'case_name' => '案件名称',
...
{
    $stmt = $pdo->prepare("SELECT id FROM patent_case_info WHERE case_code = ?");
 ...
 * 插入新专利案件
 */
function insertPatentCase($pdo, $data, $user_id)
{
    // 生成案件编号（如果为空）- 使用与add_patent.php相同的逻辑
    ...
    $sql = "INSERT INTO patent_case_info (" . implode(',', $fields) . ") VALUES (" . implode(',', $placeholders) . ")";
