...
// 专利文件管理功能 - 专利管理/案件管理模块下的文件管理功能

// 文件类型选项（与专利编辑页面保持一致）
$file_types = ['申请书', '说明书', '权利要求书', '附图', '审查意见', '答复意见', '其他'];
...
if (isset($_GET['action']) && $_GET['action'] === 'download' && isset($_GET['id']) && intval($_GET['id']) > 0) {
    ...
    try {
        $stmt = $pdo->prepare("SELECT f.*, c.case_code FROM patent_case_file f 
                              LEFT JOIN patent_case_info c ON f.patent_case_info_id = c.id 
                              WHERE f.id = ?");
...
if (isset($_GET['action']) && $_GET['action'] === 'batch_download' && isset($_GET['ids'])) {
    ...
    try {
        ...
        $zip_filename = 'patent_files_' . date('YmdHis') . '_' . uniqid() . '.zip';
        ...
        // 设置下载头信息
        $download_filename = 'patent_files_' . date('YmdHis') . '.zip';
...
if (isset($_POST['action']) && $_POST['action'] === 'delete_file') {
    ...
    try {
        ...
        // 查询文件信息
        $stmt = $pdo->prepare("SELECT * FROM patent_case_file WHERE id = ?");
        $stmt->execute([$file_id]);
        $file = $stmt->fetch();
        ...
        // 删除数据库记录
        $delete_stmt = $pdo->prepare("DELETE FROM patent_case_file WHERE id = ?");
...
if (isset($_POST['action']) && $_POST['action'] === 'batch_delete') {
    ...
    try {
        ...
        // 查询文件信息
        $placeholders = str_repeat('?,', count($file_ids) - 1) . '?';
        $stmt = $pdo->prepare("SELECT * FROM patent_case_file WHERE id IN ($placeholders)");
        $stmt->execute($file_ids);
        $files = $stmt->fetchAll();
...
if (isset($_GET['ajax']) && $_GET['ajax'] == 1) {
    ...
    try {
        ...
        // 统计总数
        $count_sql = "SELECT COUNT(*) FROM patent_case_file f 
                      LEFT JOIN patent_case_info c ON f.patent_case_info_id = c.id" . $sql_where;
...
<script>
    (function() {
        ...
        window.loadFileData = function() {
            ...
            var requestUrl = baseUrl.replace('index.php', '') + 'modules/patent_management/case_management/file_management.php';
        ...
        btnBatchDownload.onclick = function() {
            ...
            // 直接通过URL下载批量文件
            var baseUrl = window.location.href.split('?')[0];
            var downloadUrl = baseUrl.replace('index.php', '') + 'modules/patent_management/case_management/file_management.php?action=batch_download&ids=' + fileIds.join(',');
        ...
        window.downloadFile = function(fileId) {
            var baseUrl = window.location.href.split('?')[0];
            var downloadUrl = baseUrl.replace('index.php', '') + 'modules/patent_management/case_management/file_management.php?action=download&id=' + fileId;
            window.open(downloadUrl, '_blank');
        };
