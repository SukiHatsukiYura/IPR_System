...
// 来文类型
$incoming_types = ['审查意见通知书', '授权通知书', '驳回决定', '补正通知书', '缴费通知书', '年费缴费通知书', '复审通知书', '无效宣告通知书', '专利证书', '登记手续通知书', '视为撤回通知书', '恢复权利通知书', '其他官方文件'];
// 来文状态
...
// 查询所有专利案件用于下拉（限制数量以提高性能）
$patent_stmt = $pdo->prepare("SELECT id, case_code, case_name FROM patent_case_info ORDER BY case_code ASC LIMIT 1000");
$patent_stmt->execute();
$patents = $patent_stmt->fetchAll();
...
if (isset($_GET['ajax']) && $_GET['ajax'] == 1) {
    ...
    // 构建查询SQL
    $count_sql = "SELECT COUNT(*) FROM patent_incoming_document d
                  LEFT JOIN patent_case_info p ON d.patent_case_info_id = p.id" . $sql_where;
    $count_stmt = $pdo->prepare($count_sql);
    $count_stmt->execute($params);
    $total_records = $count_stmt->fetchColumn();
    $total_pages = ceil($total_records / $page_size);

    $sql = "SELECT d.*, p.case_code, p.case_name, p.application_no,
...
foreach ($patents as $patent) {
    $patents_options[$patent['id']] = $patent['case_code'] . ' - ' . $patent['case_name'];
}
...
<div class="module-panel">
    ...
    <?php render_info_notice("专利来文管理：记录和跟踪从专利局等官方机构收到的各类文件和通知，包括审查意见、授权通知、缴费通知等。支持按来文类型、状态、紧急程度等条件查询", 'info', 'icon-list'); ?>
    ...
    <div class="module-modal-content" style="width:800px;">
        ...
        <div class="module-modal-body" style="padding:20px;">
            <form id="incoming-form">
                ...
                <table class="module-table">
                    <tr>
                        <td class="module-label module-req">*专利案件：</td>
                        <td colspan="3">
                            <?php render_select_search('patent_id', $patents_options, $_GET['patent_id'] ?? ''); ?>
                        </td>
                    </tr>
                    <tr>
...
<script>
    (function() {
        ...
        window.loadIncomingData = function() {
            ...
            var requestUrl = baseUrl.replace('index.php', '') + 'modules/patent_management/case_management/patent_incoming.php';
        ...
        btnAdd.onclick = function() {
            ...
            // 重置专利下拉框
            var patentInput = incomingForm.querySelector('input[name="patent_id_display"]');
            var patentHidden = incomingForm.querySelector('input[name="patent_id"]');
            if (patentInput) patentInput.value = '';
            if (patentHidden) patentHidden.value = '';
        ...
        btnEdit.onclick = function() {
            ...
            var xhr = new XMLHttpRequest();
            var baseUrl = window.location.href.split('?')[0];
            var requestUrl = baseUrl.replace('index.php', '') + 'modules/patent_management/case_management/patent_incoming_api.php';
                ...
                if (xhr.readyState === 4 && xhr.status === 200) {
                    try {
                        ...
                        if (response.success) {
                            ...
                            // 设置专利下拉框（通用下拉框组件）
                            var patentHidden = incomingForm.querySelector('input[name="patent_id"]');
                            var patentDisplay = incomingForm.querySelector('input[name="patent_id_display"]');
                            if (patentHidden && patentDisplay) {
                                patentHidden.value = data.patent_case_info_id;
                                // 从专利选项中找到对应的显示文本
                                var patentOptions = <?php echo json_encode($patents_options); ?>;
                                if (patentOptions[data.patent_case_info_id]) {
                                    patentDisplay.value = patentOptions[data.patent_case_info_id];
                                }
                            }
        ...
        function loadUploadedFiles(documentId) {
            ...
            var xhr = new XMLHttpRequest();
            var baseUrl = window.location.href.split('?')[0];
            var requestUrl = baseUrl.replace('index.php', '') + 'modules/patent_management/case_management/patent_incoming_file_handler.php';
        ...
        window.downloadFile = function(fileId) {
            var baseUrl = window.location.href.split('?')[0];
            var downloadUrl = baseUrl.replace('index.php', '') + 'modules/patent_management/case_management/patent_incoming_file_handler.php?action=download_file&file_id=' + fileId;
        ...
        document.getElementById('btn-save-incoming').onclick = function() {
            // 验证必填字段
            var patentId = incomingForm.querySelector('input[name="patent_id"]').value;
            ...
            if (!patentId) {
                alert('请选择专利案件');
                var patentDisplay = incomingForm.querySelector('input[name="patent_id_display"]');
                if (patentDisplay) patentDisplay.focus();
                return;
            }
            if (!incomingType) {
                alert('请选择来文类型');
