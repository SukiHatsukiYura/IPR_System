...
// 已完成案件功能 - 商标管理/个人案件模块下的已完成案件管理功能
...
if (isset($_GET['ajax']) && $_GET['ajax'] == 1) {
    ...
    if ($follow_info_data) {
        // 直接查询状态为"已完成"的案件ID
        $status_stmt = $pdo->prepare("SELECT trademark_case_id FROM user_trademark_follow_case_status WHERE user_trademark_follow_id = ? AND case_status = '已完成'");
    ...
    if (empty($completed_case_ids)) {
        echo json_encode([
            'success' => true,
            'html' => '<tr><td colspan="11" style="text-align:center;padding:20px 0;">暂无已完成的案件</td></tr>',
            'total_records' => 0,
            'total_pages' => 1,
            'current_page' => 1
        ]);
...
<div class="module-panel">
    ...
    ?>
    <?php render_info_notice("已完成案件（当前用户：" . $current_user_name . "）：只显示您关注的已完成商标案件", 'success', 'icon-search'); ?>
...
<script>
    (function() {
        ...
        window.loadTrademarkData = function() {
            ...
            var requestUrl = baseUrl.replace('index.php', '') + 'modules/trademark_management/personal_cases/completed.php';
