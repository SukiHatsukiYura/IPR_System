...
// 进行中案件功能 - 商标管理/个人案件模块下的进行中案件管理功能
...
if (isset($_GET['ajax']) && $_GET['ajax'] == 1) {
    ...
    // 首先获取当前用户关注的案件ID列表
    $user_id = $_SESSION['user_id'];
    $follow_stmt = $pdo->prepare("SELECT followed_case_ids FROM user_trademark_follow WHERE user_id = ?");
    $follow_stmt->execute([$user_id]);
    $follow_data = $follow_stmt->fetch();
    ...
    if ($follow_info_data) {
        // 直接查询状态为"进行中"的案件ID
        $status_stmt = $pdo->prepare("SELECT trademark_case_id FROM user_trademark_follow_case_status WHERE user_trademark_follow_id = ? AND case_status = '进行中'");
        ...
        if (empty($in_progress_case_ids)) {
            $any_status_stmt = $pdo->prepare("SELECT COUNT(*) FROM user_trademark_follow_case_status WHERE user_trademark_follow_id = ?");
    ...
    if (empty($in_progress_case_ids)) {
        echo json_encode([
            'success' => true,
            'html' => '<tr><td colspan="11" style="text-align:center;padding:20px 0;">暂无进行中的案件</td></tr>',
            'total_records' => 0,
            'total_pages' => 1,
            'current_page' => 1
        ]);
...
<div class="module-panel">
    ...
    ?>
    <?php render_info_notice("进行中案件（当前用户：" . $current_user_name . "）：只显示您关注的进行中状态的商标案件", 'success', 'icon-search'); ?>
...
<script>
    (function() {
        ...
        window.loadTrademarkData = function() {
            ...
            var requestUrl = baseUrl.replace('index.php', '') + 'modules/trademark_management/personal_cases/in_progress.php';
