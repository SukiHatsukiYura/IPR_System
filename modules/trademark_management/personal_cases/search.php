...
if (isset($_GET['ajax']) && $_GET['ajax'] == 1) {
    ...
    // 为个人筛选添加2个相同的用户ID参数
    $personal_params = [$current_user_id, $current_user_id];
    ...
    // 合并所有参数：先是个人筛选参数，然后是搜索参数
    $all_params = $personal_params;
    ...
    $stmt = $pdo->prepare($sql);
    // 合并所有参数，并添加分页参数
    $final_params = array_merge($all_params, [$offset, $page_size]);
...
<script>
    (function() {
        ...
        window.loadTrademarkData = function() {
            ...
            var requestUrl = baseUrl.replace('index.php', '') + 'modules/trademark_management/personal_cases/search.php';
            ...
            xhr.onreadystatechange = function() {
                if (xhr.readyState === 4) {
                    ...
                    if (xhr.status === 200) {
                        console.log('原始响应:', xhr.responseText.substring(0, 200) + '...');
                        try {
                            ...
                            if (response.success) {
                                ...
                                bindTableRowClick();
                                console.log('✅ 个人商标案件数据更新完成 - 总记录数:', response.total_records);
                            } else {
                                trademarkList.innerHTML = '<tr><td colspan="10" style="text-align:center;padding:20px 0;">加载数据失败</td></tr>';
                                console.error('❌ 服务器返回失败:', response.message || '未知错误');
                            }
                        } catch (e) {
                            trademarkList.innerHTML = '<tr><td colspan="10" style="text-align:center;padding:20px 0;">加载数据失败</td></tr>';
                            console.error('❌ JSON解析失败:', e, '原始响应:', xhr.responseText);
                        }
                    } else {
        ...
        function loadTrademarkImageList() {
            ...
            var xhr = new XMLHttpRequest();
            var baseUrl = window.location.href.split('?')[0];
            var requestUrl = baseUrl.replace('index.php', '') + 'modules/trademark_management/personal_cases/search.php?ajax=image_list';
        ...
        function uploadImagesSequentially(uploadData, index) {
            ...
            var xhr = new XMLHttpRequest();
            var baseUrl = window.location.href.split('?')[0];
            var uploadUrl = baseUrl.replace('index.php', '') + 'modules/trademark_management/personal_cases/search.php';
