...
if (!isset($_GET['trademark_id']) || intval($_GET['trademark_id']) <= 0) {
    header('Content-Type: application/json');
    echo json_encode(['success' => false, 'message' => '未指定商标ID']);
    exit;
}
...
if (!$trademark_stmt->fetch()) {
    header('Content-Type: application/json');
    echo json_encode(['success' => false, 'message' => '未找到该商标信息']);
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'upload') {
        try {
            ...
            // 验证文件类型
            $allowed_types = ['商标申请书', '商标图样', '商品服务清单', '委托书', '身份证明', '优先权证明', '审查意见', '答复意见', '异议材料', '复审材料', '商标证书', '其他'];
            if (!in_array($file_type, $allowed_types)) {
                throw new Exception('无效的文件类型');
            }

            // 创建上传目录
            $upload_dir = __DIR__ . '/../../../uploads/trademark_files/' . $trademark_id . '/';
            ...
            // 保存到数据库
            $relative_path = 'uploads/trademark_files/' . $trademark_id . '/' . $final_filename;

            $stmt = $pdo->prepare("
                INSERT INTO trademark_case_file (
                    trademark_case_info_id, file_type, file_name, file_path, 
    ...
    if ($action === 'list') {
        try {
            ...
            $sql = "SELECT tcf.*, u.real_name as uploader_name 
                    FROM trademark_case_file tcf 
                    LEFT JOIN user u ON tcf.upload_user_id = u.id 
                    WHERE tcf.trademark_case_info_id = ?";
            $params = [$trademark_id];

            if (!empty($file_type)) {
                $sql .= " AND tcf.file_type = ?";
                $params[] = $file_type;
            }

            $sql .= " ORDER BY tcf.created_at DESC";
    ...
    if ($action === 'delete') {
        try {
            ...
            // 获取文件信息
            $stmt = $pdo->prepare("SELECT file_path FROM trademark_case_file WHERE id = ? AND trademark_case_info_id = ?");
            $stmt->execute([$file_id, $trademark_id]);
            ...
            // 删除数据库记录
            $stmt = $pdo->prepare("DELETE FROM trademark_case_file WHERE id = ? AND trademark_case_info_id = ?");
            $result = $stmt->execute([$file_id, $trademark_id]);
