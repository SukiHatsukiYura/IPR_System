...
// 商标文件管理功能 - 商标管理/案件管理模块下的文件管理功能

// 文件类型选项（与商标编辑页面保持一致）
$file_types = ['商标申请书', '商标图样', '商品服务清单', '委托书', '身份证明', '优先权证明', '审查意见', '答复意见', '异议材料', '复审材料', '商标证书', '其他'];
...
// 单个文件下载
if (isset($_GET['action']) && $_GET['action'] === 'download' && isset($_GET['id']) && intval($_GET['id']) > 0) {
    $file_id = intval($_GET['id']);

    try {
        $stmt = $pdo->prepare("SELECT f.*, c.case_code FROM trademark_case_file f 
                              LEFT JOIN trademark_case_info c ON f.trademark_case_info_id = c.id 
                              WHERE f.id = ?");
...
// 批量下载
if (isset($_GET['action']) && $_GET['action'] === 'batch_download' && isset($_GET['ids'])) {
    $ids_str = $_GET['ids'];
    $file_ids = array_map('intval', explode(',', $ids_str));
    ...
    try {
        ...
        $zip_filename = 'trademark_files_' . date('YmdHis') . '_' . uniqid() . '.zip';
        ...
        foreach ($files as $file) {
            ...
            if (file_exists($file_path)) {
                // 使用案件编号_文件名的格式
                $zip_filename_in_archive = ($file['case_code'] ? $file['case_code'] . '_' : '') . $file['file_name'];

                // 确保ZIP中的文件名唯一
                $counter = 1;
                $original_name = $zip_filename_in_archive;
                while ($zip->locateName($zip_filename_in_archive) !== false) {
                    $pathinfo = pathinfo($original_name);
                    $zip_filename_in_archive = $pathinfo['filename'] . '_' . $counter . '.' . $pathinfo['extension'];
                    $counter++;
                }

                $zip->addFile($file_path, $zip_filename_in_archive);
                $added_files++;
            }
        }
        ...
        // 设置下载头信息
        $download_filename = 'trademark_files_' . date('YmdHis') . '.zip';
...
if (isset($_POST['action']) && $_POST['action'] === 'delete_file') {
    ...
    try {
        ...
        // 查询文件信息
        $stmt = $pdo->prepare("SELECT * FROM trademark_case_file WHERE id = ?");
        $stmt->execute([$file_id]);
        $file = $stmt->fetch();
        ...
        // 删除数据库记录
        $delete_stmt = $pdo->prepare("DELETE FROM trademark_case_file WHERE id = ?");
...
if (isset($_POST['action']) && $_POST['action'] === 'batch_delete') {
    ...
    try {
        ...
        // 查询文件信息
        $placeholders = str_repeat('?,', count($file_ids) - 1) . '?';
        $stmt = $pdo->prepare("SELECT * FROM trademark_case_file WHERE id IN ($placeholders)");
        $stmt->execute($file_ids);
        $files = $stmt->fetchAll();
...
if (isset($_GET['ajax']) && $_GET['ajax'] == 1) {
    ...
    try {
        ...
        if (!empty($_GET['business_dept_id'])) {
            ...
            $params['business_dept_id'] = intval($_GET['business_dept_id']);
        }

        if (!empty($_GET['client_id'])) {
            $where[] = "c.client_id = :client_id";
            $params['client_id'] = intval($_GET['client_id']);
        }

        if (!empty($_GET['upload_user_id'])) {
            $where[] = "f.upload_user_id = :upload_user_id";
            $params['upload_user_id'] = intval($_GET['upload_user_id']);
        }
        ...
        // 统计总数
        $count_sql = "SELECT COUNT(*) FROM trademark_case_file f 
                      LEFT JOIN trademark_case_info c ON f.trademark_case_info_id = c.id" . $sql_where;
...
<script>
    (function() {
        ...
        window.loadFileData = function() {
            ...
            var requestUrl = baseUrl.replace('index.php', '') + 'modules/trademark_management/case_management/file_management.php';
        ...
        btnBatchDownload.onclick = function() {
            ...
            var baseUrl = window.location.href.split('?')[0];
            var downloadUrl = baseUrl.replace('index.php', '') + 'modules/trademark_management/case_management/file_management.php?action=batch_download&ids=' + fileIds.join(',');
        ...
        window.downloadFile = function(fileId) {
            var baseUrl = window.location.href.split('?')[0];
            var downloadUrl = baseUrl.replace('index.php', '') + 'modules/trademark_management/case_management/file_management.php?action=download&id=' + fileId;
            window.open(downloadUrl, '_blank');
        };
        ...
        function initSearchBoxes() {
            // 用户搜索框
            var userBoxes = document.querySelectorAll('[name="upload_user_id_display"]');
            userBoxes.forEach(function(box) {
                var container = box.closest('.module-select-search-box');
                var list = container.querySelector('.module-select-search-list');
                var listItems = list.querySelector('.module-select-search-list-items');
                ...
                function loadUsers() {
                    ...
                    <?php foreach ($users as $user): ?>
                        ...
                        item.className = 'module-select-search-item';
                        item.textContent = '<?= htmlspecialchars($user["real_name"], ENT_QUOTES) ?>';
                        item.onclick = function() {
                            box.value = '<?= htmlspecialchars($user["real_name"], ENT_QUOTES) ?>';
                ...
                searchInput.oninput = function() {
                    ...
                    var items = listItems.querySelectorAll('.module-select-search-item');
            ...
            // 部门搜索框
            var deptBoxes = document.querySelectorAll('[name="business_dept_id_display"]');
            deptBoxes.forEach(function(box) {
                ...
                function loadDepts() {
                    ...
                    <?php foreach ($departments as $dept): ?>
                        var item = document.createElement('div');
                        item.className = 'module-select-search-item';
                        item.textContent = '<?= htmlspecialchars($dept["dept_name"], ENT_QUOTES) ?>';
                        item.onclick = function() {
                            box.value = '<?= htmlspecialchars($dept["dept_name"], ENT_QUOTES) ?>';
            ...
            // 客户搜索框
            var customerBoxes = document.querySelectorAll('[name="client_id_display"]');
            customerBoxes.forEach(function(box) {
                ...
                function loadCustomers() {
                    ...
                    <?php foreach ($customers as $customer): ?>
                        var item = document.createElement('div');
                        item.className = 'module-select-search-item';
                        item.textContent = '<?= htmlspecialchars($customer["customer_name_cn"], ENT_QUOTES) ?>';
                        item.onclick = function() {
                            box.value = '<?= htmlspecialchars($customer["customer_name_cn"], ENT_QUOTES) ?>';
        ...
        // 关闭下拉框
        document.addEventListener('click', function(e) {
            if (!e.target.closest('.module-select-search-box')) {
                document.querySelectorAll('.module-select-search-list').forEach(function(list) {
                    list.style.display = 'none';
                });
            }
        });
