...
if (isset($_POST['action']) && in_array($_POST['action'], ['set_pending', 'set_completed'])) {
    ...
    try {
        ...
        foreach ($selected_case_ids as $case_id) {
            ...
            // 检查案件是否存在审核中状态的记录
            $check_stmt = $pdo->prepare("SELECT id FROM trademark_case_submission_status WHERE trademark_case_info_id = ? AND submission_status = '审核中'");
            ...
            $update_stmt = $pdo->prepare("UPDATE trademark_case_submission_status SET submission_status = ?, updated_at = NOW() WHERE trademark_case_info_id = ? AND submission_status = '审核中'");
        ...
        if ($success_count > 0) {
            ...
            echo json_encode(['success' => false, 'msg' => '没有案件被更新，请确认选中的案件当前状态为"审核中"']);
        }
    } catch (Exception $e) {
        $pdo->rollBack();
...
if (isset($_GET['ajax']) && $_GET['ajax'] == 1) {
    ...
    // 获取递交状态为"审核中"的案件ID列表
    $under_review_stmt = $pdo->prepare("SELECT DISTINCT trademark_case_info_id FROM trademark_case_submission_status WHERE submission_status = '审核中'");
...
<div class="module-panel">
    ...
    ?>
    <?php render_info_notice("审核中管理（当前用户：" . $current_user_name . "）：只显示递交状态为审核中的商标案件", 'success', 'icon-search'); ?>
...
<script>
    (function() {
        ...
        window.loadTrademarkData = function() {
            ...
            var requestUrl = baseUrl.replace('index.php', '') + 'modules/trademark_management/submission_management/under_review.php';
        ...
        btnEdit.onclick = function() {
            ...
            sessionStorage.setItem('trademark_edit_source_submenu', '1');
