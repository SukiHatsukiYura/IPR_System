...
if (isset($_POST['action']) && in_array($_POST['action'], ['set_under_review', 'set_completed'])) {
    ...
    try {
        ...
        foreach ($selected_case_ids as $case_id) {
            ...
            $update_stmt = $pdo->prepare("UPDATE trademark_case_submission_status SET submission_status = ?, updated_at = NOW() WHERE trademark_case_info_id = ? AND submission_status = '待处理'");
...
if (isset($_GET['ajax']) && $_GET['ajax'] == 1) {
    ...
    // 获取递交状态为"待处理"的案件ID列表
    $pending_stmt = $pdo->prepare("SELECT DISTINCT trademark_case_info_id FROM trademark_case_submission_status WHERE submission_status = '待处理'");
...
<div class="module-panel">
    ...
    ?>
    <?php render_info_notice("待处理管理（当前用户：" . $current_user_name . "）：只显示递交状态为待处理的商标案件", 'success', 'icon-search'); ?>
...
<script>
    (function() {
        ...
        window.loadTrademarkData = function() {
            ...
            var requestUrl = baseUrl.replace('index.php', '') + 'modules/trademark_management/submission_management/pending.php';
        ...
        btnEdit.onclick = function() {
            ...
            sessionStorage.setItem('trademark_edit_source_submenu', '0');
