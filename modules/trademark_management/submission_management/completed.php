...
if (isset($_POST['action']) && in_array($_POST['action'], ['set_pending', 'set_under_review'])) {
    ...
    if (!isset($status_map[$action])) {
        echo json_encode(['success' => false, 'msg' => '无效的操作类型: ' . $action]);
        exit;
    }
    ...
    try {
        ...
        foreach ($selected_case_ids as $case_id) {
            ...
            // 检查案件是否存在已完成状态的记录
            $check_stmt = $pdo->prepare("SELECT id FROM trademark_case_submission_status WHERE trademark_case_info_id = ? AND submission_status = '已完成'");
            ...
            $update_stmt = $pdo->prepare("UPDATE trademark_case_submission_status SET submission_status = ?, updated_at = NOW() WHERE trademark_case_info_id = ? AND submission_status = '已完成'");
        ...
        if ($success_count > 0) {
            ...
            echo json_encode(['success' => false, 'msg' => '没有案件被更新，请确认选中的案件当前状态为"已完成"']);
        }
    } catch (Exception $e) {
        $pdo->rollBack();
...
if (isset($_GET['ajax']) && $_GET['ajax'] == 1) {
    ...
    // 获取递交状态为"已完成"的案件ID列表
    $completed_stmt = $pdo->prepare("SELECT DISTINCT trademark_case_info_id FROM trademark_case_submission_status WHERE submission_status = '已完成'");
    ...
    if (empty($trademarks)) {
        ...
        foreach ($trademarks as $index => $trademark) {
            ...
            $html .= '<td>' . htmlspecialchars($trademark['trademark_class'] ?? '') . '</td>';
            $html .= '</tr>';
        }
    }
    echo json_encode([
...
<div class="module-panel">
    <div class="module-btns" style="display: flex; flex-direction: column; gap: 10px;">
        ...
        <div style="display: flex; gap: 8px; flex-wrap: wrap; margin-top: 8px;">
            ...
            <button type="button" class="btn-set-pending btn-mini" disabled style="background:#2196f3;color:#fff;border-color:#2196f3;">置为待处理</button>
    ...
    ?>
    <?php render_info_notice("已完成管理（当前用户：" . $current_user_name . "）：只显示递交状态为已完成的商标案件", 'success', 'icon-search'); ?>
...
<script>
    (function() {
        ...
        window.loadTrademarkData = function() {
            ...
            var requestUrl = baseUrl.replace('index.php', '') + 'modules/trademark_management/submission_management/completed.php';
        ...
        btnEdit.onclick = function() {
            ...
            sessionStorage.setItem('trademark_edit_source_menu', '2');
