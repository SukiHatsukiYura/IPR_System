<?php
// modules/customer_management/agency/agency_tabs/requirement.php
// 代理机构要求管理
    ...
    if ($action === 'save') {
        ...
        try {
            if ($id > 0) {
                ...
                $set = rtrim($set, ',');
                $sql = "UPDATE agency_requirement SET $set WHERE id=:id AND agency_id=:agency_id";
                ...
                $sql = "INSERT INTO agency_requirement ($fields) VALUES ($placeholders)";
                $stmt = $pdo->prepare($sql);
        ...
        $id = intval($_POST['id'] ?? 0);
        if ($id > 0) {
            $stmt = $pdo->prepare("DELETE FROM agency_requirement WHERE id=? AND agency_id=?");
        ...
        $id = intval($_POST['id'] ?? 0);
        $stmt = $pdo->prepare("SELECT * FROM agency_requirement WHERE id=? AND agency_id=?");
...
// 总数
$count_stmt = $pdo->prepare("SELECT COUNT(*) FROM agency_requirement WHERE agency_id=?");
$count_stmt->execute([$agency_id]);
$total_records = $count_stmt->fetchColumn();
$total_pages = ceil($total_records / $page_size);
// 当前页数据
$stmt = $pdo->prepare("SELECT r.*, u.real_name FROM agency_requirement r LEFT JOIN user u ON r.user_id=u.id WHERE r.agency_id=:agency_id ORDER BY r.id ASC LIMIT :offset, :limit");
$stmt->bindValue(':agency_id', $agency_id, PDO::PARAM_INT);
...
<div class="module-panel">
    <div style="margin-bottom:12px;text-align:left;">
        <button type="button" class="btn-mini" id="btn-add-requirement"><i class="icon-add"></i> 添加代理机构要求</button>
    </div>
    <table class="module-table">
        ...
        <tbody id="requirement-list">
            <?php if (empty($requirements)): ?>
                <tr>
                    <td colspan="7" style="text-align:center;color:#888;">暂无代理机构要求</td>
                </tr>
...
<div id="requirement-modal" style="display:none;position:fixed;left:0;top:0;width:100vw;height:100vh;background:rgba(0,0,0,0.18);z-index:9999;align-items:center;justify-content:center;">
    <div style="background:#fff;border-radius:10px;box-shadow:0 4px 24px rgba(0,0,0,0.18);padding:24px 32px;width:700px;max-width:98vw;max-height:80vh;position:relative;display:flex;flex-direction:column;">
        ...
        <h3 style="text-align:center;margin-bottom:18px;">代理机构要求</h3>
        ...
        function bindRequirementEvents() {
            ...
            list.onclick = function(e) {
                ...
                if (e.target.classList.contains('btn-edit-requirement')) {
                    ...
                    xhr.open('POST', 'modules/customer_management/agency/agency_tabs/requirement.php', true);
                    xhr.onload = function() {
                        try {
                        ...
                        try {
                            ...
                            if (res.success) {
                                ...
                                if (tabContent) {
                                    ...
                                    xhr2.open('GET', 'modules/customer_management/agency/agency_tabs/requirement.php?agency_id=' + form.agency_id.value, true);
            ...
            function loadRequirementPage(page, pageSize) {
                var tabContent = window.parent && window.parent.document.getElementById('agency-tab-content');
                var url = 'modules/customer_management/agency/agency_tabs/requirement.php?agency_id=' + agencyId + '&page=' + page + '&page_size=' + pageSize;
                var xhr = new XMLHttpRequest();
