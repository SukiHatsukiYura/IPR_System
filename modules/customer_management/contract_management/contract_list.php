...
if (isset($_GET['ajax']) && $_GET['ajax'] == 1) {
    ...
    // 处理合同金额范围
    if (!empty($_GET['contract_amount_min'])) {
        $where[] = "contract_amount >= :contract_amount_min";
        $params['contract_amount_min'] = floatval($_GET['contract_amount_min']);
    }
    if (!empty($_GET['contract_amount_max'])) {
        $where[] = "contract_amount <= :contract_amount_max";
        $params['contract_amount_max'] = floatval($_GET['contract_amount_max']);
    }

    // 处理签约日期范围
    if (!empty($_GET['sign_date_start'])) {
        $where[] = "sign_date >= :sign_date_start";
        $params['sign_date_start'] = $_GET['sign_date_start'];
    }
    if (!empty($_GET['sign_date_end'])) {
        $where[] = "sign_date <= :sign_date_end";
        $params['sign_date_end'] = $_GET['sign_date_end'];
    }
    ...
    $sql = "SELECT c.*, 
            (SELECT dept_name FROM department WHERE id = c.department_id) as department_name,
            ...
            (SELECT real_name FROM user WHERE id = c.responsible_user_id) as responsible_user_name
    ...
    if (empty($contracts)) {
        ...
        foreach ($contracts as $index => $contract) {
            ...
            $html .= '<td style="text-align:center;">' . intval($contract['case_count'] ?? 0) . '</td>';
            $html .= '<td>' . htmlspecialchars($contract['contract_type'] ?? '') . '</td>';
            $html .= '<td>' . htmlspecialchars($contract['contract_status'] ?? '') . '</td>';
            $html .= '<td>' . htmlspecialchars($contract['business_user_name'] ?? '') . '</td>';
            $html .= '<td>' . ($contract['sign_date'] ? date('Y-m-d', strtotime($contract['sign_date'])) : '') . '</td>';
            $html .= '</tr>';
        }
    }
    echo json_encode([
...
<div class="module-panel">
    <div class="module-btns">
        ...
        <button type="button" class="btn-add" onclick="window.parent.openTab ? window.parent.openTab(0, 3, 0) : alert('框架导航功能不可用')"><i class="icon-add"></i> 新增合同</button>
    ...
    <form id="search-form" class="module-form" autocomplete="off">
        ...
        <table class="module-table" style="margin-bottom:15px;">
            <tr>
                ...
                <td><input type="text" name="contract_no" class="module-input"></td>
                <td class="module-label">合同名称：</td>
                <td><input type="text" name="contract_name" class="module-input"></td>
                <td class="module-label">客户名称：</td>
                <td><?= render_customer_search('customer_id', $customers, '') ?></td>
            </tr>
            <tr>
                <td class="module-label">业务人员：</td>
                <td><?= render_user_search('business_user_id', $users, '') ?></td>
                <td class="module-label">合同类型：</td>
                <td><select name="contract_type" class="module-input">
                        <option value="">--全部--</option><?php foreach ($contract_types as $v): ?><option value="<?= h($v) ?>"><?= h($v) ?></option><?php endforeach; ?>
                    </select></td>
                <td class="module-label">付款方式：</td>
                <td><select name="payment_method" class="module-input">
                        <option value="">--全部--</option><?php foreach ($payment_methods as $v): ?><option value="<?= h($v) ?>"><?= h($v) ?></option><?php endforeach; ?>
                    </select></td>
            </tr>
            <tr>
                <td class="module-label">合同状态：</td>
                <td><select name="contract_status" class="module-input">
                        <option value="">--全部--</option><?php foreach ($contract_statuses as $v): ?><option value="<?= h($v) ?>"><?= h($v) ?></option><?php endforeach; ?>
                    </select></td>
                <td class="module-label">货币类型：</td>
                <td><select name="currency" class="module-input">
                        <option value="">--全部--</option><?php foreach ($currencies as $v): ?><option value="<?= h($v) ?>"><?= h($v) ?></option><?php endforeach; ?>
                    </select></td>
                <td class="module-label">所属部门：</td>
                <td><?= render_dept_search('department_id', $departments, '') ?></td>
            </tr>
            <tr>
                ...
                <td colspan="2">
                    <input type="date" name="sign_date_start" class="module-input" style="width:150px;"> 至
                    <input type="date" name="sign_date_end" class="module-input" style="width:150px;">
                </td>
            </tr>
        </table>
    </form>
    <table class="module-table">
        <thead>
            <tr style="background:#f2f2f2;">
                ...
                <th style="width:80px;text-align:center;">货币</th>
                <th style="width:80px;text-align:center;">案件数量</th>
...
<script>
    (function() {
        ...
        function loadContractData() {
            contractList.innerHTML = '<tr><td colspan="11" style="text-align:center;padding:20px 0;">正在加载数据...</td></tr>';
            selectedId = null;
            ...
            var requestUrl = baseUrl.replace('index.php', '') + 'modules/customer_management/contract_management/contract_list.php';
                    ...
                    if (xhr.status === 200) {
                        try {
                            ...
                            if (response.success) {
                                ...
                                contractList.innerHTML = '<tr><td colspan="11" style="text-align:center;padding:20px 0;">加载数据失败</td></tr>';
                            }
                        } catch (e) {
                            contractList.innerHTML = '<tr><td colspan="11" style="text-align:center;padding:20px 0;">加载数据失败</td></tr>';
                        }
                    } else {
                        contractList.innerHTML = '<tr><td colspan="11" style="text-align:center;padding:20px 0;">加载数据失败，请稍后重试</td></tr>';
                    }
                }
            };
            xhr.send();
