...
if (isset($_GET['ajax']) && $_GET['ajax'] == 1) {
    ...
    // 草稿条件：合同状态为空或者是"未开始"
    $where[] = "(contract_status IS NULL OR contract_status = '' OR contract_status = '未开始')";
    ...
    $html = '';
    if (empty($contracts)) {
        $html = '<tr><td colspan="8" style="text-align:center;padding:20px 0;">暂无草稿数据</td></tr>';
    } else {
...
<script>
    (function() {
        ...
        function loadContractData() {
            contractList.innerHTML = '<tr><td colspan="8" style="text-align:center;padding:20px 0;">正在加载数据...</td></tr>';
            selectedId = null;
            ...
            var requestUrl = baseUrl.replace('index.php', '') + 'modules/customer_management/contract_management/draft.php';
                    ...
                    if (xhr.status === 200) {
                        try {
                            ...
                            if (response.success) {
                                ...
                                contractList.innerHTML = '<tr><td colspan="8" style="text-align:center;padding:20px 0;">加载数据失败</td></tr>';
                            }
                        } catch (e) {
                            contractList.innerHTML = '<tr><td colspan="8" style="text-align:center;padding:20px 0;">数据解析失败</td></tr>';
                        }
                    } else {
                        contractList.innerHTML = '<tr><td colspan="8" style="text-align:center;padding:20px 0;">网络请求失败，请稍后重试</td></tr>';
                    }
                }
            };
            xhr.send();
