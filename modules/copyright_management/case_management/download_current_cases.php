...
function buildWhereCondition($params)
{
    ...
    if (!empty($params['is_subsidy_agent'])) {
        $where[] = "c.is_subsidy_agent = ?";
        $values[] = ($params['is_subsidy_agent'] === '是') ? 1 : 0;
    }
...
// 获取版权案件数据
function getCopyrightCases($pdo, $params = [])
{
    ...
    $sql = "SELECT c.id,
                ...
                c.is_expedited,
                c.is_subsidy_agent,
                c.is_material_available,
                c.remarks
            FROM copyright_case_info c
...
// 获取案件数据
$cases = getCopyrightCases($pdo, $searchParams);

// 版权案件基本信息表的字段（必填字段放在左侧，id固定第一列）
$headers = [
    // 第1列：数据库主键ID（固定第一列，禁止修改）
    'id' => 'id',
    // 第2列：我方文号（可选，留空则自动生成）
    'case_code' => '我方文号(可编辑，会自动生成)',
...
$filename = "版权案件信息{$current_date}.xls";
...
foreach ($cases as $case) {
    ...
    foreach ($headers as $field => $header) {
        ...
        if ($field === 'client_id') {
            ...
            // 特殊字段处理
            if ($field === 'is_subsidy_agent' || $field === 'is_material_available') {
                $value = $value ? '1' : '0';
            } elseif (in_array($field, ['entrust_date', 'open_date', 'application_date', 'registration_date', 'expire_date'])) {
                // 日期字段格式化
...
if (empty($cases)) {
    ...
    echo '<Data ss:Type="String">没有找到符合条件的版权案件数据</Data>' . "\n";
    echo '</Cell>' . "\n";
    echo '</Row>' . "\n";
}
