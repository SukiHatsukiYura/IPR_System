...
try {
    ...
    // 批量修改数据
    $result = batchUpdateCopyright($pdo, $rows, $header_map, $_SESSION['user_id']);
...
function getHeaderMapping($headers)
{
    ...
    $header_mapping = [
        ...
        '案源国' => 'source_country',
        '开卷日' => 'open_date',
        '开卷日(YYYY-MM-DD)' => 'open_date',
        '受理号' => 'application_no',
        '受理日' => 'application_date',
        '受理日(YYYY-MM-DD)' => 'application_date',
        '登记号' => 'registration_no',
        '登记日' => 'registration_date',
        '登记日(YYYY-MM-DD)' => 'registration_date',
        '证书号' => 'certificate_no',
        '届满日' => 'expire_date',
        ...
        '是否代办资助(1是0否)' => 'is_subsidy_agent',
        '有无材料' => 'is_material_available',
        '有无材料(1有0无)' => 'is_material_available',
        '案件备注' => 'remarks'
    ];
 ...
 * 批量修改版权案件
 */
function batchUpdateCopyright($pdo, $rows, $header_map, $user_id)
{
    ...
    $current_cases_data = [];
    if (!empty($case_ids)) {
        $current_cases_data = getCurrentCopyrightDataBatch($pdo, $case_ids);
    ...
    try {
        foreach ($rows as $row_index => $row) {
            ...
            try {
                ...
                // 解析行数据
                $copyright_data = parseCopyrightRow($pdo, $row, $header_map, $departments, $customers, $users);

                // 调试信息：记录解析后的数据
                error_log("第{$line_number}行解析后数据: " . json_encode($copyright_data, JSON_UNESCAPED_UNICODE));

                // 验证必须包含id字段
                if (empty($copyright_data['id']) || !is_numeric($copyright_data['id'])) {
                    $errors[] = "第{$line_number}行: 缺少有效的id字段，无法确定要修改的案件";
                    $error_count++;
                    continue;
                }

                $case_id = intval($copyright_data['id']);

                // 检查案件是否存在
                $stmt = $pdo->prepare("SELECT id FROM copyright_case_info WHERE id = ?");
                $stmt->execute([$case_id]);
                if (!$stmt->fetch()) {
                    $errors[] = "第{$line_number}行(案件ID:{$case_id}): 该版权案件不存在";
                    $error_count++;
                    continue;
                }

                // 如果用户填写了客户ID，验证客户ID是否存在并检查版权案件类型
                if (isset($copyright_data['client_id']) && !empty($copyright_data['client_id']) && is_numeric($copyright_data['client_id'])) {
                    $stmt = $pdo->prepare("SELECT id, case_type_copyright FROM customer WHERE id = ?");
                    $stmt->execute([$copyright_data['client_id']]);
                    $customer = $stmt->fetch();

                    if (!$customer) {
                        $errors[] = "第{$line_number}行(案件ID:{$case_id}): 客户ID {$copyright_data['client_id']} 不存在，请填写有效的客户ID";
                        $error_count++;
                        continue;
                    }
                    ...
                    if ($customer['case_type_copyright'] == 0) {
                        try {
                            ...
                            $stmt->execute([$copyright_data['client_id']]);
                            error_log("为客户ID {$copyright_data['client_id']} 添加版权案件类型");
                        } catch (Exception $e) {
                            error_log("更新客户ID {$copyright_data['client_id']} 版权案件类型失败: " . $e->getMessage());
                        }
                    }
                }

                // 验证必填字段（只验证有值的字段）
                $validation_result = validateCopyrightData($copyright_data, $line_number, $case_id);
                ...
                $update_result = updateCopyrightCaseOptimized($pdo, $copyright_data, $case_id, $current_data);
 ...
 * 验证版权数据
 */
function validateCopyrightData($data, $line_number, $case_id = null)
{
    // 定义必填字段
    $required_fields = [
        'case_name' => '案件名称',
...
/**
 * 更新版权案件（性能优化版：只更新发生变化的字段）
 */
function updateCopyrightCase($pdo, $data, $case_id)
{
    // 移除id字段，因为不能更新主键
    unset($data['id']);

    // 首先获取当前数据库中的数据
    $current_data = getCurrentCopyrightData($pdo, $case_id);
    if (!$current_data) {
    ...
    foreach ($data as $field => $new_value) {
        ...
        // 特殊处理日期字段的比较
        if (in_array($field, ['entrust_date', 'open_date', 'application_date', 'registration_date', 'expire_date'])) {
        ...
        // 特殊处理布尔字段的比较
        if (in_array($field, ['is_subsidy_agent', 'is_material_available'])) {
    ...
    $sql = "UPDATE copyright_case_info SET " . implode(', ', $changed_fields) . " WHERE id = ?";
 ...
 * 获取当前案件数据
 */
function getCurrentCopyrightData($pdo, $case_id)
{
    $sql = "SELECT * FROM copyright_case_info WHERE id = ?";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$case_id]);
 ...
 * 批量获取当前案件数据（性能优化）
 */
function getCurrentCopyrightDataBatch($pdo, $case_ids)
{
    if (empty($case_ids)) {
        return [];
    }
    ...
    $sql = "SELECT * FROM copyright_case_info WHERE id IN ({$placeholders})";
    $stmt = $pdo->prepare($sql);
 ...
 * 优化版更新函数（使用预加载的数据）
 */
function updateCopyrightCaseOptimized($pdo, $data, $case_id, $current_data = null)
{
    // 移除id字段，因为不能更新主键
    unset($data['id']);
