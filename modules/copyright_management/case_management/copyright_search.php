...
<div id="batch-update-modal" class="module-modal" style="display:none;">
    <div class="module-modal-content" style="width:600px;">
        <div class="module-modal-header">
            <h3 class="module-modal-title">批量修改版权案件</h3>
            <button class="module-modal-close">&times;</button>
        </div>
        <div class="module-modal-body" style="padding:20px;">
            <div style="margin-bottom:20px;">
...
<script>
    (function() {
        ...
        btnDownloadCurrent.onclick = function() {
            ...
            var baseUrl = window.location.href.split('?')[0];
            var downloadUrl = baseUrl.replace('index.php', '') + 'modules/copyright_management/case_management/download_current_cases.php';
                ...
                if (xhr.status === 200) {
                    try {
                        ...
                        if (response.success) {
                            ...
                            resultHtml += '<br><br><button class="btn-theme" onclick="loadCopyrightData(); closeBatchImportModal();">刷新列表并关闭</button></div>';
        ...
        btnStartUpdate.onclick = function() {
            ...
            // 发送请求
            var xhr = new XMLHttpRequest();
            xhr.open('POST', 'modules/copyright_management/case_management/batch_update.php', true);
                ...
                if (xhr.status === 200) {
                    try {
                        ...
                        if (response.success) {
                            ...
                            resultHtml += '<br><br><button class="btn-theme" onclick="loadCopyrightData(); closeBatchUpdateModal();">刷新列表并关闭</button></div>';
                            resultDiv.innerHTML = resultHtml;
                        } else {
